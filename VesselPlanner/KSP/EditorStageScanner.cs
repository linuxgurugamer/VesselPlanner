...
namespace VesselPlanner.KSP
{
    public static class EditorStageScanner
    {
        ...
        // Returns the current editor craft's full wet mass, including resources.
        // False means there is no vessel in the editor, so callers can use their
        // own empty-editor default without confusing an empty craft with a 0 t craft.
        public static bool TryGetVesselWetMassTons(out double wetMassTons)
        {
            wetMassTons = 0.0;
            if (EditorLogic.fetch == null || EditorLogic.fetch.ship == null || EditorLogic.fetch.ship.parts == null) return false;
        ...
        // Returns the stack profile at the open lower end of the editor vessel. Starting
        // at the root and following attached "bottom" nodes tracks the central stack to
        // its lower end. If that final part has no bottom node, its top node is used.
        public static bool TryGetVesselBottomOrTopBulkheadProfile(out string profile)
        {
            profile = null;
            try
            {
                if (EditorLogic.fetch == null || EditorLogic.fetch.ship == null ||
                    EditorLogic.fetch.ship.parts == null || EditorLogic.fetch.ship.parts.Count == 0)
                    return false;

                List<Part> parts = EditorLogic.fetch.ship.parts;
                Part current = parts.FirstOrDefault(p => p != null);
                if (current == null) return false;

                var visited = new HashSet<Part>();
                while (current != null && visited.Add(current))
                {
                    AttachNode bottom = current.FindAttachNode("bottom");
                    if (bottom != null)
                    {
                        if (bottom.attachedPart != null && parts.Contains(bottom.attachedPart) && !visited.Contains(bottom.attachedPart))
                        {
                            current = bottom.attachedPart;
                            continue;
                        }

                        if (bottom.size >= 0)
                        {
                            profile = ResolveNodeBulkheadProfile(current, bottom);
                            return !string.IsNullOrEmpty(profile);
                        }
                    }

                    AttachNode top = current.FindAttachNode("top");
                    if (top != null && top.size >= 0)
                    {
                        profile = ResolveNodeBulkheadProfile(current, top);
                        return !string.IsNullOrEmpty(profile);
                    }
                    break;
                }
            ...
            {
                UnityEngine.Debug.LogWarning("[VesselPlanner] Unable to determine editor vessel bulkhead profile: " + ex.Message);
            }
            return false;
        }

        private static string ResolveNodeBulkheadProfile(Part part, AttachNode node)
        {
            if (node == null || node.size < 0) return null;
            string fallback = BulkheadProfiles.ProfileForSize(node.size);
            string profiles = part != null && part.partInfo != null ? part.partInfo.bulkheadProfiles : null;
            if (string.IsNullOrEmpty(profiles)) return fallback;

            var stackProfiles = profiles.Split(',')
        ...
        private static void TryApplyStockStageMasses(ExistingStageSnapshot snap, ShipConstruct ship, int stage, IEnumerable<Part> selectedEngineParts)
        {
            ...
            {
                UnityEngine.Debug.LogWarning("[VesselPlanner] Stock stage-mass lookup failed; using fallback scanner: " + ex.Message);
            }
        }
