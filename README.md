...
- In **Planning**, the horizontal grip above **Selected Engine** and **Tanks** trades height between the two rows. Dragging down enlarges the **Requirements** pane and the candidate engine list, which follows it because the list is aligned to the bottom of that box; dragging up shrinks them and enlarges the **Selected Engine** and **Tanks** panes instead. Every pane scrolls in a fixed viewport and the two rows always add up to the same total, so the drag never changes the window size; it stops where either pane would reach its limit.
...
- 0.5.58 stops the Planning grip expanding the window when dragged down.
- 0.5.57 makes the Planning grip enlarge the Selected Engine and Tanks panes when dragged up.
